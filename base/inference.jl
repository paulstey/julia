...
function finish(me::InferenceState)
    ...
    if me.optimize
        ...
        meta_elim_pass!(me.linfo, me.linfo.code::Array{Any,})
...
function meta_elim_pass!(linfo::LambdaInfo, code::Array{Any,1})
    ...
    #    * `Expr(:inbounds, false)` in two `Expr(:inbounds, false)`
    #
    #    If we aren't going to inline any functions anymore, which is always the
    #    case now, we can also delete.
    #
